...
// ============================================================================
// 0x Gasless API Constants
// ============================================================================

/**
 * Chain IDs for 0x API
 */
export const OX_CHAIN_IDS: Record<string, number> = {
  'ethereum': 1,
  'polygon': 137,
  'arbitrum': 42161,
  'optimism': 10,
...
/**
 * Networks that support 0x Gasless API
 * Reference: https://0x.org/docs/gasless-api/introduction
 */
export const GASLESS_SUPPORTED_NETWORKS: string[] = [
  'ethereum',
  'polygon', 
  'arbitrum',
  'optimism',
...
/**
 * Helper: Check if a network supports 0x Gasless swaps
 */
export function isGaslessSupported(network: string): boolean {
  return GASLESS_SUPPORTED_NETWORKS.includes(network);
}

/**
 * Helper: Get chain ID for 0x API
 */
export function get0xChainId(network: string): number | null {
  return OX_CHAIN_IDS[network] || null;
}

/**
 * Minimum gas balance (in wei) required to execute a regular swap
 * If user has less than this, we should try gasless first
